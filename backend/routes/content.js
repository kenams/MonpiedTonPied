...
const hasPurchase = async (user, contentId) => {
    ...
    return Boolean(await Purchase.findOne({ user: user._id, content: contentId }));
};
...
const signMediaUrl = (contentId, index) => {
    const token = signToken({ c: contentId.toString(), i: index, exp: Date.now() + MEDIA_TOKEN_TTL_MS });
...
function formatFeedItem(item, currentUser, purchasedIds, canAccessAll) {
    ...
    const purchased = purchasedIds ? purchasedIds.has(item._id.toString()) : false;
    const unlocked = isOwner || canAccessAll || purchased;

    const files = (item.files || []).map((file, index) => ({
        url: (!unlocked && index > 0) ? null : (getUploadsPath(file.url) ? signMediaUrl(item._id, index) : file.url),
        type: file.type,
    ...
    return {
        ...
        price: item.price || 0,
        locked: !unlocked,
        stats: item.stats || { views: 0, likes: 0 },
        liked: currentUser ? (item.likedBy || []).map(id => id.toString()).includes(currentUser._id.toString()) : false,
    };
}

// ─── GET /liked — contenus likés par l'utilisateur ───────────────────────────
router.get('/liked', auth, async (req, res) => {
    try {
        const items = await Content.find({ likedBy: req.user.id })
        ...
        return res.json({
            items: items.map(item => ({
                ...
                stats: item.stats,
                creator: { _id: item.creator?._id, username: item.creator?.username, displayName: item.creator?.displayName, avatarUrl: item.creator?.avatarUrl },
                locked: false,
            }))
        });
    } catch (error) {
...
// ─── GET /feed — feed paginé avec catégorie/tag + reco IA ────────────────────
router.get('/feed', optionalAuth, async (req, res) => {
    try {
        ...
        if (recommended === 'true' && currentUser && !category && !tag) {
            ...
            return res.json({
                items: sliced.map(item => formatFeedItem(item, currentUser, purchasedIds, canAccessAll)),
                hasMore: skip + sliced.length < docs.length,
                total: docs.length,
            });
        }
        ...
        const purchasedIds = new Set(
            currentUser
                ? (await Purchase.find({ user: currentUser._id, content: { $in: docs.map(d => d._id) } })).map(p => p.content.toString())
                : []
        );

        return res.json({
            items: docs.map(item => formatFeedItem(item, currentUser, purchasedIds, canAccessAll)),
...
// ─── GET / — liste (usage interne/dashboard) ─────────────────────────────────
router.get('/', optionalAuth, async (req, res) => {
    try {
        const currentUser = req.currentUser || (req.user ? await User.findById(req.user.id) : null);
        const canAccessAll = hasPremiumAccess(currentUser);
        const filter = {};
        if (req.query.mine === 'true' && currentUser) filter.creator = currentUser._id;
        else if (req.query.creator) {
            const c = await User.findOne({ username: req.query.creator });
            if (c) filter.creator = c._id;
        }
        const items = await Content.find(filter).sort({ createdAt: -1 }).limit(200)
            .populate('creator', 'username displayName avatarUrl role verifiedCreator');
        const purchasedIds = new Set(
            currentUser
                ? (await Purchase.find({ user: currentUser._id, content: { $in: items.map(i => i._id) } })).map(p => p.content.toString())
                : []
        );
        return res.json({ items: items.map(item => formatFeedItem(item, currentUser, purchasedIds, canAccessAll)) });
    } catch (error) {
...
router.get('/:id', optionalAuth, async (req, res) => {
    try {
        const currentUser = req.currentUser || (req.user ? await User.findById(req.user.id) : null);
        const item = await Content.findById(req.params.id).populate('creator', 'username displayName avatarUrl role');
        if (!item) return res.status(404).json({ message: 'Contenu introuvable.' });
        const creatorId = item.creator?._id?.toString();
        const isOwner = currentUser && creatorId === currentUser._id.toString();
        const canAccessAll = hasPremiumAccess(currentUser);
        const purchased = await hasPurchase(currentUser, item._id);
        const previewItems = await Content.find({ creator: item.creator?._id }).sort({ createdAt: -1 }).limit(PREVIEW_LIMIT).select('_id');
        const previewIds = new Set(previewItems.map(d => d._id.toString()));
        ...
        return res.json({
            ...
            creator: { id: creatorId, username: item.creator?.username || 'Anonyme', displayName: item.creator?.displayName || item.creator?.username || 'Anonyme', avatarUrl: item.creator?.avatarUrl || '/default-avatar.svg' },
            files: item.files.map((file, index) => {
                const isPreviewFile = isPreview && index === 0;
                const isLocked = !canAccess && !isPreviewFile;
                return { ...(file.toObject ? file.toObject() : file), url: isLocked ? null : (getUploadsPath(file.url) ? signMediaUrl(item._id, index) : file.url), isLocked };
            }),
            canAccess, isPreview, stats: item.stats,
        });
    } catch (error) {
...
router.post('/:id/like', auth, async (req, res) => {
    try {
        ...
        if (alreadyLiked) { item.likedBy.pull(userId); item.stats.likes = Math.max(0, (item.stats.likes || 0) - 1); }
        else { item.likedBy.push(userId); item.stats.likes = (item.stats.likes || 0) + 1; }
        await item.save();
...
router.post('/', auth, async (req, res) => {
    try {
        ...
        if (role !== 'creator' && role !== 'admin') return res.status(403).json({ message: 'Compte créateur requis.' });
        ...
        return res.status(201).json({ _id: content._id, title: content.title, creator: { id: user._id, username: user.username }, files: content.files, tags: content.tags, category: content.category, price: content.price, stats: content.stats });
    } catch (error) {
